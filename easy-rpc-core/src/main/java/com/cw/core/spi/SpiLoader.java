...
public class SpiLoader {
    ...
    public static <T> T getInstance(Class<?> tClass, String key) {
        ...
        if (keyClassMap == null) {
            throw new RuntimeException(String.format("SpiLoader 未加载 %s", tClassName));
        }
        if (!keyClassMap.containsKey(key)) {
            throw new RuntimeException(String.format("SpiLoader %s 不存在类型 key=%s", tClassName, key));
        }
    ...
    public static Map<String, Class<?>> load(Class<?> loadClass) {
        ...
        for (String scanDir : SCAN_DIRS) {
            //注意:获取配置文件是用ResourceUtil.getResources获取的,如果框架作为依赖被注入,是无法正确获取到正确的文件路径的
            ...
            for (URL resource : resources) {
                try {
                    ...
                    rLog.error("spi resource load error", e);
                    throw new RuntimeException("spi resource load error", e);
                }
            }
        }
